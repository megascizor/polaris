    ...
    )
{
    ...
    for(st_index=0; st_index<param_ptr->num_st; st_index++){
        ...
        for(ch_index=0; ch_index<MAX_CH_VIEW; ch_index++){
            plot_y[ch_index] = 10.0* log10(spec_ptr[st_index* MAX_CH_VIEW + ch_index]); // Auto-correlation in dB
        }
        ...
        cpgsvp(
            0.067+xwin_incr*nx_index, 0.067+xwin_incr*(nx_index+0.9),
    ...
    sprintf(text, "%04d %03d %02d:%02d:%02d\0", param_ptr->year, param_ptr->doy, param_ptr->hour, param_ptr->min, param_ptr->sec);
    cpgsci(1); cpgtext(x_text, y_text, text);
