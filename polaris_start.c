//
// Start Polaris-Related Processes
//
...
int main(int argc, char **argv)
{
    ...
    int         bandWidth=4;          // Bandwidth [MHz]: 2/4/8/16/32/64/128/256/512/1024
    int         qbit=8;               // Quantization bits: 1/2/4/8
    ...
    //     return(-1);
    // }
    // printf("polaris_start: PID (2) = %d\n", pid);
    shrd_param_id = shmget( SHM_PARAM_KEY, sizeof(struct SHM_PARAM), 0666);
    ...
    printf("polaris_start: PARAM SET SEGNUM = %d \n", param_ptr->segNum);
    printf("polaris_start: PARAM SET FSAMPLE = %d \n", param_ptr->fsample);
    ...
    // printf("param_ptr->sem_data/id = %d\n" ,param_ptr->sem_data_id);
    // printf("polaris_start: PID (3) = %d\n", pid);
    ...
    if(param_ptr->validity & PGPLOT ){
        ...
        //         perror("Can't Create Chiled Process!!\n"); return(-1);
        //     }
        // }
        //
        // if(fork() == 0){
        ...
        if(fork() == 0){
            pid = getpid(); sprintf(cmd[0], "shm_xspec_view");
            printf(" Exec %s as Chiled Process [PID = %d]\n", cmd[0], pid);
            if(execl( XSPEC_VIEW, cmd[0], cmd[1], (char *)NULL) == -1){
                perror("Can't Create Chiled Process!!\n"); return(-1);
            }
        }

        if(fork() == 0){
            pid = getpid(); sprintf(cmd[0], "shm_phase_view");
            printf("Exec %s as Chiled Process [PID = %d]\n", cmd[0], pid);
            if(execl( PHASE_VIEW, cmd[0], cmd[1], (char *)NULL) == -1){
                perror("Can't Create Chiled Process!!\n"); return(-1);
            }
        }
    }
