//
// Phase of Cross-power Spectra Monitor
//
    ...
    )
{
    ...
    for(st_index = 0; st_index < param_ptr->num_st / 2; st_index++){
        ...
        for(ch_index = 0; ch_index < MAX_CH_VIEW; ch_index++){
            plot_y[ch_index] = atan2(spec_ptr[st_index* MAX_CH_VIEW + 2*ch_index+1], spec_ptr[st_index* MAX_CH_VIEW + 2*ch_index]);
        }
        ...
        cpgsci(3); cpgpt(MAX_CH_VIEW, freq_ptr, plot_y, 3);

        // IF number
        x_text = xmin * 0.3 + xmax * 0.7;
        y_text = ymin * 0.1 + ymax * 0.9;
        sprintf(text, "IF=%d Peak%7.2f rad", st_index, peakVal);
        cpgsci(3);
