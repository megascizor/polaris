    ...
    )
{
    ...
    inCHnum = param.num_ch; outCHnum = inCHnum / bunchNum; 
    if(inCHnum % bunchNum){
        outCHnum ++;
    }
    switch(filetype){
        ...
        case 1:
            ...
            outSize = outCHnum* sizeof(float);
            break;
        case 2:
            recSize = inCHnum* sizeof(double);  // Crosscorr file
            outSize = outCHnum* sizeof(double);
            break;
        case 3:
            recSize = 0;
    ...
    if(recSize == 0){
        perror("Unavailable Data Format");
        return(-1);
    }
    ...
    for(index=StartPP; index<=EndPP; index++){
        ...
        if(filetype == 1){
            bunchReal(inCHnum, bunchNum, recdata, outdata);
        }
        if(filetype == 2){
            bunchComplex(inCHnum, bunchNum, recdata, outdata);
        }
